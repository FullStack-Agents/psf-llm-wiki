...
When a full node first joins the network or returns from being offline, it must synchronize its local [blockchain](blockchain.md) to match the network's current state (source: mastering-bitcoin-cash_chapter-5-the-bitcoin-cash-network_4.md).

## The Synchronization Workflow

The process starts by comparing [blockchain](blockchain.md) heights across peers using the `BestHeight` field in the initial `version` message (source: mastering-bitcoin-cash_chapter-5-the-bitcoin-cash-network_4.md).
...
4. **Incremental Addition**: As blocks are received, they are added to the local [blockchain](blockchain.md), and the process repeats until the node reaches the current network height (source: mastering-bitcoin-cash_chapter-5-the-bitcoin-cash-network_4.md).
