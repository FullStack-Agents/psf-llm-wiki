...
**Summary**: This document explains the mathematical foundations of [bitcoin-cash](bitcoin-cash.md) ownership, specifically focusing on elliptic curve cryptography and the derivation process from private keys to addresses.
...
The security of [bitcoin-cash](bitcoin-cash.md) relies on [public key](addresses.md) cryptography, specifically mathematical functions that are "one-way" (easy to calculate but hard to reverse). 
...
`Private Key (k)` $\to$ `Elliptic Curve Multiplication` $\to$ `Public Key (K)` $\to$ `Cryptographic Hash Function` $\to$ `[bitcoin-cash](bitcoin-cash.md) Address (A)`
