...
Some [bitcoin-cash](bitcoin-cash.md) client implementations maintain a **[utxo](utxo.md) database** (or [utxo](utxo.md) pool) that keeps track of all unspent transaction outputs currently on the [blockchain](blockchain.md) (source: mastering-bitcoin-cash_chapter-5-the-bitcoin-cash-network_9.md).
...
## Role in Validation
The [utxo](utxo.md) database is critical for efficient transaction validation. Instead of scanning the entire [blockchain](blockchain.md) for every transaction, a node uses the [utxo](utxo.md) pool to quickly verify:
