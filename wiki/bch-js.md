...
`bch-js` is a JavaScript library maintained by the [Permissionless Software Foundation](permissionless-software-foundation.md) that provides a developer-friendly API for communicating with the [psf-bch-api]([psf-bch-api](psf-bch-api.md).md) REST API server (source: bch-js.md).
...
1. **[x402](x402-bch.md) Pay-Per-Call**: Uses [x402-bch]([x402](x402-bch.md)-bch.md) to handle micro-payments per API call automatically using a WIF [private key](private-keys.md).
