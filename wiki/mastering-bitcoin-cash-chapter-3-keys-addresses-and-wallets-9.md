...
- The resulting 512-bit hash is split:
    ...
    - **Left 256 bits**: Combined with the parent private key to produce the child private key.

### Hardened Derivation
To increase security, HD wallets use "hardened" derivation (denoted by a prime symbol, e.g., $m/0'$). This breaks the mathematical link between a parent public key and child chain codes, ensuring that a compromised child key cannot be used to reveal other keys in the tree.

### BIP0044 Standard
BIP0044 defines a standardized path for HD wallets to ensure interoperability:
`m / purpose' / coin_type' / account' / change / address_index`

For Bitcoin Cash, the `coin_type` is `145'`. For example, the path `M/44'/145'/0'/0/2` represents the third receiving address for the primary account.
