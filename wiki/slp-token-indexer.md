...
**Summary**: A blockchain indexer that tracks SLP token transactions according to Type 1 and NFT1 specifications, providing the necessary data for token-related operations in the Cash Stack.
...
The **SLP Token Indexer** crawls the blockchain to index SLP token transactions, similar to how the [fulcrum-indexer](fulcrum-indexer.md) indexes BCH addresses and balances (source: slp-indexer-software.md). It specifically tracks tokens following the Type 1 and NFT1 specifications (source: slp-indexer-software.md).

Together with the [bchn-full-node](bchn-full-node.md) and the [fulcrum-indexer](fulcrum-indexer.md), it forms the core back-end infrastructure of the Cash Stack (source: slp-indexer-software.md).
...
2. **Block Indexer**: Handles the Initial Block Download (IBD) starting from the SLP Genesis (block ~543,000). Once IBD is complete, it monitors the full node's ZMQ interface for new blocks and creates periodic database backups (source: slp-indexer-software.md).
...
The SLP indexer requires a fully synced [bchn-full-node](bchn-full-node.md) to operate (source: slp-indexer-software.md). A full sync from SLP Genesis can take **two to three weeks** (source: slp-indexer-software.md). 
